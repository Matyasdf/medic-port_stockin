...
"""
Integration code for dispensing.py

Add these imports and endpoints to your existing main.py
"""
...
@app.post("/dispense/create", tags=["Dispensing Operations"])
async def create_dispense_task(request: DispenseRequest):
    ...
    {
        "products": [
            {"barcode": "ABC123", "quantity": 2},
            {"barcode": "XYZ789", "quantity": 1}
        ],
        "output_id": null  // Optional: specify output (null = auto-select nearest per trip)
    }

    Response:
    {
        "status": "success",
        "task_id": "DISP-20251112-0001",
        "total_items": 3,
        "total_trips": 2,
        "robots_assigned": ["R1", "R2"],
        "instructions": [
            {
                "robot_id": "R1",
                "trip_number": 1,
                "output_position": {"x": 650, "y": 50, "z": 0},
                "items": [
                    {
                        "item_id": 123,
                        "barcode": "ABC123",
    ...
    Note: Robots execute in trip_number order (1, 2, 3, ...). Up to 2 items per trip.
    """
    global task_counter, relocate_tasks_store
    ...
    # Update global task_counter and relocate_tasks_store
    task_counter = updated_task_counter
    relocate_tasks_store.update(updated_relocate_tasks)
    return response

@app.post("/dispense/complete", tags=["Dispensing Operations"])
async def complete_dispense(request: CompleteDispenseRequest):
    """
    Mark a dispense task as complete
    
    This will:
    - Remove items from inventory
    - Update dispense logs (daily/weekly tracking)
    - Free up robots
...
@app.post("/dispense/fail", tags=["Dispensing Operations"])
async def fail_dispense(request: FailDispenseRequest):
    """
    Mark a dispense task as failed
    ...
    Response:
    {
        ...
        "robots_freed": ["R1", "R2"],
        "failure_reason": "Robot malfunction",
        "failed_at": "2025-11-12T10:35:00"
    }
    """
...
@app.get("/dispense/logs", tags=["Dispensing Logs & History"])
async def get_dispense_logs():
    ...
    Response:
    {
        "status": "success",
        "summary": {
            "total_products_tracked": 50,
            "total_dispenses_all_time": 1523,
            "dispenses_today": 45,
            "dispenses_this_week": 312,
...
@app.get("/dispense/product/{product_id}", tags=["Dispensing Logs & History"])
async def get_product_dispense_history(product_id: int):
    ...
    Response:
    {
        ...
        "last_dispensed": "2025-11-12T10:30:00",
        "daily_dispenses": [
            {"date": "2025-11-12", "quantity": 15},
            {"date": "2025-11-11", "quantity": 12}
        ],
        "weekly_dispenses": [
            {"week": "2025-W46", "quantity": 67},
            {"week": "2025-W45", "quantity": 83}
        ]
    }
    """
    return get_product_dispense_history_endpoint(product_id)

@app.get("/dispense/task/{task_id}", tags=["Dispensing Logs & History"])
async def get_task_status(task_id: str):
    ...
    Response:
    {
        ...
        "created_at": "2025-11-12T10:25:00",
        "completed_at": "2025-11-12T10:30:00",
        "failure_reason": null,
        "items_count": 3,
        "robots": ["R1", "R2"]
